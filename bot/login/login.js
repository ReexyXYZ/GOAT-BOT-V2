...
let subTitle = `GoatBot V2@${currentVersion} - A simple bot chat messenger using personal account`;
const subTitleArray = [];
...
const author = "Created by mi with ♡";
const srcUrl = "Source code: https://github.com/";
...
app.use(express.static(path.join(__dirname, "../../Nazrul")));
        const PORT = global.GoatBot.config.dashBoard?.port || (!isNaN(global.GoatBot.config.serverUptime.port) && global.GoatBot.config.serverUptime.port) || process.env.PORT || 3001;
        app.get("/", (req, res) => {
  res.sendFile(path.join(__dirname, "../../Nazrul", "index.html"));
});
