...
async function downloadAndSendVideo(videoUrl, api, event) {
  try {
    ...
    const { data } = await axios.get(`${await xApi()}/ytMp4?url=${encodeURIComponent(videoUrl)}`);
...
module.exports.onStart = async ({ api, event, args }) => {
  ...
  try {
    if (!videoUrl) {
      ...
      const searchResponse = await axios.get(`${await xApi()}/ytSearch?query=${encodeURIComponent(query)}`);
